      ...
      <div className="flex flex-col gap-4 overflow-y-auto max-h-[600px] w-full">
          ...
          {Object.keys(filters).map(
            (filterType) =>
              ...
              filters[filterType].map((value) => (
                ...
                >
                  {filterType}:{" "}
                  {filterType === "price" ? priceDisplayMap[value] : value}
                  {filterType !== "tag" && (
                    <button
        ...
        <div>
          <Brands
            ...
            checkedState={checkedState.brand}
          />
        </div>
        {/* Catalogue Section */}
        <div>
          <Catalogue
            handleFilterChange={handleFilterChange}
            checkedState={checkedState.catalogue}
          />
        </div>
        {/* Size Section */}
        <div>
          <Sizes
            handleFilterChange={handleFilterChange}
            checkedState={checkedState.size}
          />
        </div>
        {/* Colour Section */}
        <div>
          <Colors
            handleFilterChange={handleFilterChange}
            checkedState={checkedState.color}
          />
        </div>
        {/* Price Range Section */}
        <div>
          <Price
            handleFilterChange={handleFilterChange}
            checkedState={checkedState.price}
          />
        </div>
      </div>
    </div>
