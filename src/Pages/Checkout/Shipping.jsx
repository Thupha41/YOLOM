...
const Shipping = () => {
    ...
    const { removeFromCart, totalCartAmount, getTotalCartItems, cartItems } = useContext(ShopContext);
    ...
    const addressValidation = (address) => {
        const regex = /^([0-9A-ZẮẰẲẴẶĂẤẦẨẪẬÂÁÀÃẢẠĐẾỀỂỄỆÊÉÈẺẼẸÍÌỈĨỊỐỒỔỖỘÔỚỜỞỠỢƠÓÒÕỎỌỨỪỬỮỰƯÚÙỦŨỤÝỲỶỸỴ.,/\s]+){2,}$/i;
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Assuming you have saved the IDs in localStorage
                const savedCityId = localStorage.getItem('city_id');
                const savedDistrictId = localStorage.getItem('district_id');
                const savedWardId = localStorage.getItem('ward_id');
                
                if (savedCityId) {
                    setSelectedProvince(savedCityId);
                    const foundProvince = response.data.find(province => province.Id === savedCityId);
                    if (foundProvince) {
                        setDistricts(foundProvince.Districts);
                    }
                    if (savedDistrictId) {
                        setSelectedDistrict(savedDistrictId);
                        const foundDistrict = foundProvince?.Districts.find(district => district.Id === savedDistrictId);
                        if (foundDistrict) {
                            setWards(foundDistrict.Wards);
                        }
                        if (savedWardId) {
                            setSelectedWard(savedWardId);
                        }
                    }
                }
            } catch (error) {
    ...
    const handleProvinceChange = (event) => {
        ...
        const foundProvince = provinces.find(province => province.Id === selectedId);
    ...
    const handleDistrictChange = (event) => {
        ...
        const foundDistrict = districts.find(district => district.Id === selectedId);
    ...
    const renderCartItems = () => {
        if (cartItems.length === 0) {
            return (
                <div className="flex flex-col items-center">
                    ...
                    <p className="mt-2 text-xl text-center font-semibold text-white">Your cart is empty!</p>
                </div>
            );
        }
        return cartItems.map((item) => {
            ...
            return (
                <div key={product.sku_id} className="grid sm:grid-cols-2 items-start gap-6">
                    <div className="px-0 py-0 shrink-0 bg-gray-50 rounded-md">
                        <Link to={`/product/${item.ProductDetail.sku_slug}`} key={item.ProductDetail.sku_id} onClick={() => { window.scrollTo(0, 0); }}><img src={product.sku_image[0]} className="w-full object-cover h-[260px]" alt={product.Product.product_name} /></Link>
                    </div>
                    <div>
                        <h3 className="text-base text-white">{product.Product.Brand.name}</h3>
                        <h4 className="text-[12px] text-white">{product.Product.product_name}</h4>
                        <ul className="text-xs text-white space-y-3 mt-4">
                            <li>Color: {product.sku_color}</li>
                            <li>Size: {product.sku_size}</li>
                            <li>Price: <span className='text-red-500'>{formatNumber(product.Product.product_price)}đ</span></li>
                            <li>Quantity:</li>
                            <li><QuantityUpdate id={product.sku_id}/> </li>
    ...
    //Restore data from localStorage whenever component load
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        if (selectedProvince) {
            const foundProvince = provinces.find(province => province.Id === selectedProvince);
            if (foundProvince) {
                setDistricts(foundProvince.Districts);
                if (selectedDistrict) {
                    const foundDistrict = foundProvince.Districts.find(district => district.Id === selectedDistrict);
                    if (foundDistrict) {
                        setWards(foundDistrict.Wards);
                    }
                }
    ...
    //update state whenever user input value
    const handleInputChange = (field, value) => {
        ...
        localStorage.setItem('checkout-data', JSON.stringify(updatedData));
    };
    ...
    const handleShippingForm = (e) => {
        ...
        if (!checkoutData.shippingAddressFormData.first_name) {
            ...
            hasError = true;
        } else if(!nameValidation(checkoutData.shippingAddressFormData.first_name)) {
            setErrFirstName ("Invalid");
        }
        if (!checkoutData.shippingAddressFormData.last_name) {
            setErrLastName("Required");
            hasError = true;           
        } else if(!nameValidation(checkoutData.shippingAddressFormData.last_name)) {
            setErrLastName ("Invalid");
        }
        if (!checkoutData.shippingAddressFormData.email_field) {
            setErrEmail("Required");
            hasError = true;
        } else if (!emailValidation(checkoutData.shippingAddressFormData.email_field)) {
            setErrEmail("Invalid");
        ...
        if(!checkoutData.shippingAddressFormData.phone) {
            ...
            setErrPhone("Enter a valid phone number. It should start with 0 and contain 11 numbers");
        ...
        if(!checkoutData.shippingAddressFormData.address) {
            ...
            hasError = true;
        } else if (!addressValidation(checkoutData.shippingAddressFormData.address)) {
                        ...
                        <div className="bg-[#3f3f3f] lg:h-screen lg:sticky lg:top-0">
                            <div className="relative h-full">
                                <div className="p-8 lg:overflow-auto lg:h-[calc(100vh-60px)]">
                                    ...
                                    <h3 className="text-xl font-bold text-white"> ({getTotalCartItems()} items)</h3>
                                    <div className="space-y-6 mt-10">
                                        {renderCartItems()}
                                    </div>
                                </div>
                                <div className="absolute left-0 bottom-0 bg-[#444] w-full p-4">
                                    <h4 className="flex flex-wrap gap-4 text-base text-white">Total <span className="ml-auto">{formatNumber(totalCartAmount)} đ</span></h4>
                                </div>
                            </div>
                        </div>
                        {/* Right side */}
                        <div className="xl:col-span-2 h-max rounded-md p-8 sticky top-0">
                            ...
                            <form className="mt-10">
                                ...
                                <div>
                                    ...
                                    {formError && <div style={{ color: 'red' }} className='mb-4'>{formError}</div>}
                                    <div className="grid sm:grid-cols-2 gap-6">
                                        ...
                                        <div className="relative flex items-center">
                                            <input 
                                                ...
                                                className={`px-4 py-3.5 bg-white text-[#333] w-full text-sm border-b-2 focus:border-[#333] outline-none ${errFirstName ? 'border-red-600' : ''}`}
                                                value={checkoutData.shippingAddressFormData.first_name}
                                                onChange={(e) => { handleInputChange(e.target.name, e.target.value); setErrFirstName(''); }} 
                                        ...
                                        <div className="relative flex items-center">
                                            <input
                                                ...
                                                className={`px-4 py-3.5 bg-white text-[#333] w-full text-sm border-b-2 focus:border-[#333] outline-none ${errLastName ? 'border-red-600' : ''}`}
                                                value={checkoutData.shippingAddressFormData.last_name} 
                                                onChange={(e) => { handleInputChange(e.target.name, e.target.value); setErrLastName(''); }}
                                            />
                                            {errLastName ? (
                                        ...
                                        <div className="relative flex items-center">
                                            <input
                                                ...
                                                type="email" 
                                                placeholder="Email *"
                                                className={`px-4 py-3.5 bg-white text-[#333] w-full text-sm border-b-2 focus:border-[#333] outline-none ${errEmail ? 'border-red-600' : ''} `} 
                                                value={checkoutData.shippingAddressFormData.email_field}
                                                onChange={(e) => { handleInputChange(e.target.name, e.target.value); setErrEmail(''); }}
                                            />
                                            {errEmail ? (
                                                <p className="text-red-500 text-sm font-titleFont font-semibold mt-1 px-2 flex items-center gap-1">
                                                    <span className="text-sm italic font-bold">!</span>
                                        ...
                                        <div className="relative flex items-center">
                                            <input 
                                                ...
                                                className={`px-4 py-3.5 bg-white text-[#333] w-full text-sm border-b-2 focus:border-[#333] outline-none  ${errPhone ? 'border-red-600' : ''}`}
                                                value={checkoutData.shippingAddressFormData.phone} 
                                                onChange={(e) => { handleInputChange(e.target.name, e.target.value); setErrPhone(''); }}
                                            />
                                            {errPhone ? (
                                                <p className="text-red-500 text-sm font-titleFont font-semibold mt-1 px-2 flex items-center gap-1">
                                                    <span className="text-sm italic font-bold">!</span>
                                ...
                                <div className="mt-6">
                                    ...
                                    <div className="grid sm:grid-cols-2 gap-6">
                                        ...
                                        {/* Province */}
                                        <select 
                                            className={`px-4 py-3.5 bg-white text-[#333] w-full text-sm border-b-2 focus:border-[#333] outline-none ${provinceError ? 'border-red-600' : ''}`}
                                        ...
                                        {/* District */}
                                        <select 
                                            className={`px-4 py-3.5 bg-white text-[#333] w-full text-sm border-b-2 focus:border-[#333] outline-none ${districtError ? 'border-red-600' : ''}`}
                                        ...
                                        {/* Ward */}
                                        <select 
                                            className={`px-4 py-3.5 bg-white text-[#333] w-full text-sm border-b-2 focus:border-[#333] outline-none ${wardError ? 'border-red-600' : ''}`} 
                                            disabled={!selectedDistrict || wards.length === 0} 
                                            value={selectedDistrict && wards.length > 0 ? checkoutData.shippingAddressFormData.ward_id : ''}
                                        ...
                                        <div className="flex items-center">
                                            <input
                                                ...
                                                className={`px-4 py-3.5 bg-white text-[#333] w-full text-sm border-b-2 focus:border-[#333] outline-none  ${errAddress ? 'border-red-600' : ''}`}
                                                value={checkoutData.shippingAddressFormData.address}
                                                onChange={(e) => { handleInputChange(e.target.name, e.target.value); setErrAddress(''); }} 
                                            ...
                                            {errAddress ? (
                                                ...
                                                <svg className="w-[18px] h-[18px] dark:text-white absolute right-12" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="#bbb" viewBox="0 0 18 20">
                                    ...
                                    <div className='mt-6'>
                                        <h3 className="text-lg font-bold text-[#333] mb-6">Order Notes</h3>
                                        <div className="w-full mb-4 border border-gray-200 rounded-lg bg-gray-50 dark:bg-gray-700 dark:border-gray-600">
                                            <div className="px-4 py-2 bg-white rounded-t-lg">
                                                <label htmlFor="comment" className="sr-only">Your comment</label>
                                                <textarea  onChange={handleMessages}  value={messages} id="comment" rows="4" className="w-full px-0 text-sm text-gray-900 bg-white border-0 dark:bg-gray-800 focus:ring-0 dark:text-white" placeholder="Write a note..." required ></textarea>                                       
                                            </div>
                                        </div>
                                    </div>
                                    {/* Buttons */}
                                    <div className="flex gap-6 max-sm:flex-col mt-10">
                                        <button type="button" className="rounded-md px-6 py-3 w-full text-sm font-semibold bg-transparent hover:bg-gray-100 border-2 text-[#333]" onClick={handleReturnToCart}>Return to cart</button>
                                        <button type="button" className="rounded-md px-6 py-3 w-full text-sm font-semibold bg-[#333] text-white hover:bg-[#222]" onClick={handleShippingForm}>Continue</button>
                                    </div>
                                </div>
                            </form>
                        </div>
